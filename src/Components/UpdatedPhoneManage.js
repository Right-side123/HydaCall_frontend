...
const UpdatedManagePhone = () => {
    const [historyPhone, setHistoryPhone] = useState(false);
    const [updatedSimNumbers, setUpdatedSimNumbers] = useState([]);
    ...
    const fetchUpdatedSimNumbers = async () => {
        try {
            ...
            const res = await api.get('/updatedsimnumber');
            setUpdatedSimNumbers(res.data);
        } catch (err) {
    ...
    const historySIMNumber = async (sim) => {
        try {
            if (!sim) return;
            await api.get(`/updatedsimnumber/${sim.id}`, {
                params: { Name: sim.Name, Department: sim.Department }
            });
    ...
    const filteredSimNumbers = updatedSimNumbers.filter(sim =>
    ...
    const handleSort = (key) => {
        ...
        setUpdatedSimNumbers(sortedData);
    };
                                    ...
                                    currentHistoryPage.map((sim, index) => (
                                        <tr key={sim.id}>
                                            ...
                                            <td style={{ textAlign: 'center', display: ' flex', justifyContent: 'center' }}>
                                                <div style={{ width: '30px', height: '30px', backgroundColor: '#e3effaff', display: 'flex', alignItems: 'center', padding: '0px', justifyContent: 'center' }}>

                                                    <img src={eyeUserIcon} alt='history' style={{ width: '20px', height: '18px' }} onClick={() => handleEditClick(sim)} />
                                                </div>
                    ...
                    <div className='main_container_model'>
                        <div className='manage_phone_model_container'>
                            <div className='manage_phone_model_title'>
                                <h3>Changes</h3>
                                <img src={crossIcon} alt='back' className='model_cross_icon' onClick={() => setHistoryPhone(false)} />
                            </div>
...
export default UpdatedManagePhone;
